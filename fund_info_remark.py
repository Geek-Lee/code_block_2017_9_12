...
# >>> a
# {'manager_name': 'manager_name', 'manager_resume': 'manager_resume'}
# >>> b = [a]
# >>> b
# [{'manager_name': 'manager_name', 'manager_resume': 'manager_resume'}]
# >>> info_data = DataFrame()
# >>> info_data['manager_info'] = b
# >>> info_data
#                                         manager_info
# 0  {'manager_name': 'manager_name', 'manager_resu...
# >>> info_data.to_dict(orient="record")
# [{'manager_info': {'manager_name': 'manager_name', 'manager_resume': 'manager_resume'}}]
# >>> info_data.to_dict(orient="record")[0]
# {'manager_info': {'manager_name': 'manager_name', 'manager_resume': 'manager_resume'}}
# >>> result = {}
# >>> result['fid'] = info_data.to_dict(orient="record")[0]
# >>> result
# {'fid': {'manager_info': {'manager_name': 'manager_name', 'manager_resume': 'manager_resume'}}}
# >>>


fetch_fund_info('JR000001')
# SELECT fia.fund_id,
# DATE_FORMAT(fia.statistic_date, '%%Y-%%m-%%d') AS nav_date,
# fia.fund_name,
# fia.fund_full_name,
# fia.type_code_name_3 AS fund_type_issuance,
# concat_ws('-', fia.type_code_name_1, fia.stype_code_name_1) AS fund_type_strategy,
# DATE_FORMAT(fia.foundation_date, '%%Y-%%m-%%d') AS foundation_date,
# fia.type_code_name_4 AS fund_type_structure,
# fia.region, fia.fund_status, fia.fund_time_limit, fia.open_date, fia.data_freq, fia.fund_stockbroker, fia.fund_custodian, fia.fee_subscription, fia.expected_return, fia.fee_redeem, fia.fee_manage,
# fia.fee_trust,
# DATE_FORMAT(fia.reg_time, '%%Y-%%m-%%d') AS reg_time,
# fia.fee_pay,
# mi.user_name AS fund_manager,
# mi.resume AS manager_info,
# oi.profile` AS org_info,
# fia.org_full_name AS org_name,
# fas.asset_scale
# FROM fund_info_aggregation fia
# 	LEFT JOIN fund_manager_mapping fmm ON fmm.fund_id = fia.fund_id
# 		AND fmm.is_leader = 1
# 		AND fmm.is_current = 1
# 	LEFT JOIN manager_info mi ON fmm.user_id = mi.user_id
# 	LEFT JOIN org_info oi ON oi.org_id = fia.org_id
# 	LEFT JOIN fund_asset_scale fas ON fas.fund_id = fia.fund_id
# 		AND fas.statistic_date IN (SELECT MIN(statistic_date)
# 			FROM fund_asset_scale
# 			WHERE fund_id = fia.fund_id)
# WHERE fia.fund_id = id
